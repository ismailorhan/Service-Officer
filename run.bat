...
"%PY%" -c "import sys; from PySide6.QtWidgets import QApplication; from core import config, state; from ui import panel, theme; a=QApplication(sys.argv); cfg=config.load(); theme.set_mode(cfg.theme); a.setStyleSheet(theme.sheet()); w=panel.MainPanel(cfg, store=state.store); w.resize(1060,700); w.show(); sys.exit(a.exec())"
exit /b %errorlevel%
